 <?php 
 	global $wpdb;
					   $now = date("Y-m-d");
						  $month = date('m');
						  $year = date('Y');
						  if (isset($_REQUEST['sfdate'])) :
						     $activedate = $_REQUEST['sfdate'];
					...
					$querystr = "
						...
						AND year($wpdb->postmeta.meta_value) = '$activeyear' 
						GROUP BY month($wpdb->postmeta.meta_value)
					...
					<div class="accordion">
					 ...
					 $activemonth = date("m",strtotime($events->startdate));
					 ?>
				         <h3 class="title-event"><?php echo date("F Y",strtotime($events->startdate));?> - <em><?php echo $events->countevent;?> <?php if ($events->countevent > 1): echo "Events"; else : echo "Event"; endif;?></em></h3>
                        <div class="content-event clearfix">
						<?php 
						 
					 $x=1;
					 $m = date("m",strtotime($events->startdate));
					 $y = date("Y",strtotime($events->startdate));
					 ...
					 AND wpostmeta.meta_key = 'startdate' AND month(wpostmeta.meta_value) = '$m'
					 AND YEAR(wpostmeta.meta_value) = '$y'
...
//						$monthstartdate = date("m",strtotime($startdate));

//						if (($activeyear == $yearstartdate ) && ($activemonth == $monthstartdate ) ): 						
